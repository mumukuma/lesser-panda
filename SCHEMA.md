...
isb_checked: 2026-08-15   # 選填（2026-08-18 起）：**這隻的整筆 ISB 紀錄已與條目逐欄對過帳**的日期（多次對帳記最後一次）。與 `studbook_id` 分工——有番號只代表「ISB 查得到號碼」，**對帳與否一律看本欄**；ISB 全簿查無者也可填（＝已查過、結論是 ISB 沒這隻，此時 `studbook_id` 留空、本欄有值）。對帳細節仍寫在內文 `✅ **ISB … 對帳（日期）**` 註記，本欄只是機器可讀的旗標，讓維護者與 Claude 一眼看出哪些不必再驗一次。`pnpm dev` 的個體頁「編號」列顯示成 `ISB 97106 ✅2026-08-15 · RPF #591`；正式站與 `audit.py` 均不使用，缺值不是資料缺陷（多數條目本來就還沒對）
tags: [標籤]
...
- **官方但不對外呈現、僅 dev mode 顯示的來源**：兩類——①（2026-08-11 維護者裁定）ISB《International Red Panda Studbook》（Rotterdam Zoo／Diergaarde Blijdorp 編），原檔 2012 年已由園方下架，公開可指的只剩 Wayback 快照；②（2026-08-20 維護者裁定）**一切 Wayback 快照**（`web.archive.org` 等），含園方舊頁存檔（のいち舊個體頁、平川舊 blog、静岡市舊 PDF、池田已 404 訊息等）。兩類**都是官方一手來源、佐證權重照舊**（`has_official_source` 照算、🚧 解除照舊），但**正式站個體頁不列出連結**，只在 `pnpm dev` 的「來源」卡片以 🔒＋「僅 dev 顯示」列出（與「編號」列同一招 `import.meta.env.DEV`，正式 build 的 HTML 完全不含這些網址）。連結原封不動留在 frontmatter `sources:` 供校訂稽核，ISB 原檔存 `sources/isb-red-panda/`。實作：`tools/build_db.py` 的 `is_public_source()`（外層 host 是 Wayback 直接判非公開；其餘比對 `NON_PUBLIC_HOSTS`）→ 拆進 DB 的 `sources_private` 欄 → `export_json.py` 匯出為 `pandas.json` 的 `sources_dev`（該檔僅建置期由 `data.js` 讀、不進 `web/dist`，安全）→ `Panda.astro` 的 dev-only 清單。⚠️ 這是「官方但不公開」，與「非官方」是兩回事，**勿把 `rotterdamzoo.nl` 從 `OFFICIAL_HOSTS` 移除**；⚠️ `sources_dev` 別加進 `searchDataFor`／`ZOOS_DATA` 之類內嵌進 HTML 的資料。
