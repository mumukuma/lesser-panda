...
### 資料有限個體：檔案卡（2026-07-17 起，主要用於中國個體；2026-07-24 放寬門檻）

中國園常無公告即轉移個體、不公布生日與家系、也不公告死亡，等資料齊全再建檔等於永遠不建。改採「**證據快照**」模式：記錄「確認過什麼、何時確認」，不宣稱全貌。

**建檔門檻（2026-07-24 放寬；三項皆符合才建）**：

1. **知道名字與性別**（性別不確定則暫緩，先記入 `cn-candidates.json`）。
2. **綁定至少一座已登記園**（該園須已登記於 `data/zoos.json`）——**硬門檻**。
3. **至少一項佐證**：官方來源／讀者實拍的園方展牌（記 `extra_sources`）／**或維護者親自確認的社群流通資訊**（`sources` 寫 `維護者提供（YYYY-MM-DD）`）。
...
- **親屬關係不能替代綁園**：只知「牠是某隻的孩子」卻不知在哪個園 → 不建檔，寫進對方 `## 家族` 敘述 + 記 `cn-candidates.json`。關係僅為輔助佐證。

只有名字、一次目擊、來源開不了、或綁不上園的 → 不建條目，先記入候補名單 `data/cn-candidates.json`（見下），累積到符合門檻再轉正。
...
- `sex:` **必填**（門檻），不確定就暫緩建檔、先進 `cn-candidates.json`
- `sources:` 有官方就填官方；**純維護者確認則寫 `維護者提供（YYYY-MM-DD）`**（無 host、不會被當官方來源）。讀者實拍展牌放 `extra_sources`
