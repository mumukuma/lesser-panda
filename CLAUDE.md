...
- **地點（`location_*`）也以 `data/zoos.json` 為準**：`gen_residence.py` **只補空白、永不覆寫**既有校訂值（2026-06-29 起；收割對象自 2026-08-09 起是 `location_zh`，因為收割來源是中文內文表格）。要改某園地點 → 直接編輯 `data/zoos.json` 再重建；內文居住史的「地點」欄由註冊表自動帶入，勿手改。
- **⚠️ 地點三欄一欄一語，不可混填**（2026-08-09 全庫統一，這是最常被寫錯的一組欄位）：
  - `location_ja`＝**當地語言**（日本園日文新字体「神奈川県横浜市」；中／港／澳／台／韓／新加坡園寫日文可讀的漢字或片假名「上海市長寧区」「香港南区黄竹坑」「シンガポール」）。**非漢字圈的園沒有日文地點就留空**（`null`），別把中文塞進來——ja 語系會自動退回 `location_en`，顯示英文遠比顯示中文合理。
  - `location_zh`＝**繁體中文正本**（「神奈川縣橫濱市」「紐約州水牛城」）。簡體由建置期 `toHans` 轉，不另存。
  - `location_en`＝英文。
  - 常見繁中⇄日文對應字：縣⇄県、區⇄区、橫⇄横、濱⇄浜、靜⇄静、廣⇄広、繩⇄縄、兒⇄児、戶⇄戸、姬⇄姫、德⇄徳、豐⇄豊、寶⇄宝、澤⇄沢、莊⇄荘、關⇄関、錢⇄銭、黃⇄黄。
  - **取用順序**（`web/src/lib/data.js` 的 `zooLocation`）：zh-TW／zh-CN→`zh`→`ja`→`en`；ja→`ja`→`en`；en／ko→`en`→`ja`。wiki 內文居住史表格（`gen_residence.py`）走 `zh`→`ja`→`en`，因為內文是中文。
  - 新增一座園時**三欄一起想**：先問「當地怎麼寫」（`ja`，非漢字圈就留空）、「繁中怎麼寫」（`zh`）、「英文怎麼寫」（`en`）。從 lineage／RPF 帶入的地點常是繁中或英文，落地前先分流，別整串丟進 `location_ja`。也要順手看語序（lineage 會給出「佐世保市長崎県」這種顛倒值）。
...
3. **尚在世**（公告時已夭折者不建蘋果籽；建檔後於佔位期間夭折的處理見下方「**佔位期間夭折**」——2026-07-21 起改為當季保留、季末才下架，非即時移 `_hidden`）
...
**佔位期間夭折（2026-07-21 起，覆蓋 2026-07-02 舊做法）**：
...
（活著但未命名的蘋果籽不受此影響，照下方「季末盤點」10 月起查命名。此為 2026-07-14「佔位名者移 `_hidden`」規則對蘋果籽的時機細化：改為季末才移、非即時。）

**季末盤點**：每年 10 月起以 `grep -l "apple-seed" wiki/*.md` 盤點尚未轉正的蘋果籽，逐隻查園方是否已公布名字；跨年仍未命名者留佔位、不強改。

### 資料有限個體：檔案卡（2026-07-17 起，主要用於中國個體；2026-07-24 放寬門檻；2026-08-02 性別門檻再放寬）

中國園常無公告即轉移個體、不公布生日與家系、也不公告死亡，等資料齊全再建檔等於永遠不建。改採「**證據快照**」模式：記錄「確認過什麼、何時確認」，不宣稱全貌。

**建檔門檻（2026-08-02 更新；三項皆符合才建）**：

1. **知道名字**，且**性別已知 _或_ 有已建條目的家系連結**（詳見下方「性別門檻」）。
2. **綁定至少一座已登記園**（該園須已登記於 `data/zoos.json`）——**硬門檻**。
3. **至少一項佐證**：官方來源／讀者實拍的園方展牌（記 `extra_sources`）／**或維護者親自確認的社群流通資訊**（`sources` 寫 `維護者提供（YYYY-MM-DD）`；讀者回報則另記 `讀者回報（@帳號，佐證方式，YYYY-MM-DD）`）。

**性別門檻（2026-08-02 放寬，覆蓋舊「`sex` 必填」規則）**：

- 原則仍是**性別已知才建檔**；只有名字、性別不明、又無其他線索的 → 進 `cn-candidates.json`。
- **例外：有「父／母／子女／配偶」其中一方已建條目的家系連結時，性別可留空建檔。** 家系線索比單純名字目擊紮實得多，且把這類個體卡在候補名單，會讓對方（尤其種公種母）的子女表殘缺不全、家系圖斷鏈。
- 性別留空時：`sex:` 空白、**tags 不加性別 tag**、內文標「性別待確認」、index 性別欄寫「不詳」。DB 記為 `sex: unknown`，**不計入任何性別統計**（同蘋果籽佔位條目的處理），總數與園頁照常顯示。
- 之後查到性別即回補 `sex` ＋ 性別 tag、移除「性別待確認」字樣（log 記 `update`）。
- **先試著從關係語推定性別**，推得出就不必留空：「父親是X」→ X ♂；「母親是Y」→ Y ♀；「Z 的老公」→ Z ♀、該個體 ♂。但**關係語只描述對方、推不出本人性別時不可硬猜**（例：「黑帥是心心的孩子」只給出心心 ♂，黑帥本人性別仍不詳；名字帶「帥」「妹」等字**不足以推定**）。

**放寬要點與紅線（2026-07-24）**：

- **官方來源不再是必要條件**——維護者親自確認（看過社群流通名單、讀者實拍、親自比對）即可建檔，維護者的確認本身就是 wiki 權威的一環。此舉把 `xiao-xiao-bai-nanjing` 等既成實務正式寫進政策。
- **紅線：嚴禁僅憑 RPF/lineage 機械資料、無人確認就建檔**——那類仍只是線索，續留 `cn-candidates.json`。放寬的真正邊界是「有無維護者/人為確認」，不是「有無官方來源」。
- **親屬關係不能替代綁園**：只知「牠是某隻的孩子」卻不知在哪個園 → 不建檔，寫進對方 `## 家族` 敘述 + 記 `cn-candidates.json`。關係僅為輔助佐證。**注意這條與上方性別例外不衝突**：家系連結可以替代「性別」門檻，但**永遠不能**替代「綁園」硬門檻。

只有名字、一次目擊、來源開不了、綁不上園、或（性別不明**且**無家系連結）的 → 不建條目，先記入候補名單 `data/cn-candidates.json`（見下），累積到符合門檻再轉正。
...
- `sex:` 原則必填；**有已建條目的家系連結時可留空**（見上方「性別門檻」），此時 tags 不加性別 tag。兩者皆無則暫緩建檔、先進 `cn-candidates.json`
- `sources:` 有官方就填官方；**純維護者確認則寫 `維護者提供（YYYY-MM-DD）`**（無 host、不會被當官方來源）。**讀者回報經維護者採用者寫兩行**：`讀者回報（@帳號，佐證方式，YYYY-MM-DD）` ＋ `維護者提供（YYYY-MM-DD）`，前者記真實出處、後者記維護者確認（即建檔依據），兩者皆無 host 故 `has_official_source` 仍為 false。讀者實拍展牌放 `extra_sources`
- `zoos:` 只登**有佐證**出現過的園；**起始年預設留空（渲染為「? – 現在」）**（2026-07-24 起，覆蓋舊「一律填首次確認年份、不可留空」規則）：檔案卡個體多半只知道「某時點確認在園」、並不知道真正的入園／出生年，硬填「首次確認年份」會在網站顯示成看似真實的入園年（如「2026 – 現在」），污染入園年份統計、誤導讀者。故**不知抵達年就留空**，比照 `pei-pei`／`kang-kang`／`xue-xue` 等既有廣州條目寫 `園名 ( – 現在)`。**唯一已知副作用**：`gen_residence` 會給空白起始的首站標 🐣 出生地符號（即使非出生園，如野生救護個體）——這是可接受的既有慣例（老條目皆如此），優先於顯示假年份。**只有確知是出生園／確知抵達年**才填該年（此時 🐣 才名實相符）。動向不明者現居訖同樣可留空（配合 `unverified`）；**已故或已確認離園**者訖填死亡日／該園最後確認日（起始仍留空，如 `( – 2025)`、`( – 2026-02-22)`）
- `rpf_id`／`rpf_url`：中國個體 RPF 覆蓋率低，通常缺（audit 提示不擋），之後有再補

**兩軸分工（勿混用；2026-07-24）**：個體有兩條獨立的可信度軸——

- **佐證軸**：這筆紀錄有無官方背書。**自動推導、不手動掛 tag**：`build_db` 的 `sources` 欄本就只留官方來源，`export_json` 據此輸出 `has_official_source` 旗標；`維護者提供（…）` 無 host 故不算官方 → 旗標為 false，網站顯示「維護者提供・未經官方佐證」標記、來源區塊自然留空。日後補進官方來源，旗標自動轉 true、標記自動消失。
- **在世軸**：這隻動向明不明，由 `unverified` 手動掛。**切勿為觸發標記而濫掛 `unverified`**，否則會把「在世但無官方佐證」誤標成「動向不明」、污染在世統計。「維護者確認、現正在世」的新個體＝`has_official_source: false` + **不掛** `unverified`。
...
**盤點**：不定期以 `grep -l "limited-profile" wiki/*.md` 盤點，逐隻搜園方微信有無新目擊／死訊／轉園；有新確認就更新 `last_seen`（log 記 `update`）。另可用 `grep -l "^sex:$" wiki/*.md` 盤點性別留空的檔案卡（會一併列出蘋果籽，以 tags 區分），查得性別即回補。
...
2. **先把 `born` 釘死，再開檔**（2026-08-25 起）：`born` 是 slug 的一部分，寫錯要走 `tools/rename.py`
   全站遷移，其他欄位寫錯只是原地改一行。來源優先序＝**出生園當時的一手來源**（新聞稿／園報／官方社群）
   ＞ 收容園日後撰寫的介紹頁 ＞ RPF。回報者或現居園給的生日，**先核一次出生園的公告**再落檔；
   核不到就照用並在備注寫「僅 X 為據」。
   找的順序：現行官網／官方社群 → 該園既有管道（園報／staff blog／年度存檔）→ **Wayback**
   （頁面 404／改版下架／園已閉園時的**正規手段**，快照仍算官方一手；先例：茶臼山年度存檔、
   池田訃報、のいち舊個體頁、平川舊 blog、静岡市舊 PDF）。三層**各搜一輪，沒有就停**，
   比照下方〈查證省時原則〉。
   ⛔ **可以直接跳過的只有兩種**：① 出生園不詳（沒有搜索目標）；② 該園該年代確實沒有對外
   紀錄管道**且專案已確認過**（如釧路《あゆみ》未數位化、安佐飼育記録集掃描無文字層）——
   第一次遇到某座園不算，先搜一輪再把結論記進 memory。
   ⛔ **年代不是判準**：全庫 200 筆以 Wayback 為來源的條目裡，169 筆生年在 2000 年前——
   年代早正是要翻 Wayback 的理由，別用年代線一刀切。
   ⚠️ 兩個官方來源打架時**先分辨誰在場、誰在事後轉述**——下方〈官方來源可直接採用〉的「官方之間衝突
   留待維護者裁定」是給**同級**來源用的，一手 vs 事後轉述不同級，可自行判斷並把差異寫進備注
   （實例：Pavitra 生日，ZooMontana 官網記 6/1、出生園聖地牙哥新聞稿記 6/9，採 6/9）。
3. 建立 `wiki/[slug].md`
4. **補齊直系親屬——只展開一圈**（2026-08-25 改，覆蓋舊的「祖父母也一律建立」）：
   - 建：父、母、同胎手足、兄弟姊妹、子女、**配偶**（子女的另一位親本；不建會讓全血緣手足被判成 ½）
   - **不建**：祖父母，以及上述新建條目自己的父母／子女／手足——**不遞迴**，一律寫純文字＋RPF ID
   - 每一隻另需過門檻：**`born` 有可寫成 slug 的粒度**（完整日期或年份）**且有正式名字**；門檻沒過就純文字
   - 範圍外撈到的個體（祖父母、新的一胎、同園其他隻）**列進回報問維護者，不擅自建**
   - 順序：主角→父→母→同胎手足→子女→配偶
5. 回頭把相關既有條目的純文字親屬改成 `[[wikilink]]`
6. 更新 `wiki/index.md`：加入適當分類、更新頁首「最後更新」與「條目總數」
7. 在 `wiki/log.md` 末端 append 一筆記錄
8. 重跑 `python3 tools/build_db.py`
9. 跑 `python tools/audit.py` 檢查資料完整度（缺欄位等；lineage 比對預設不跑，需要時加 `--lineage`）；網站資料則重跑 `python3 pipeline/scripts/export_json.py`
...
- **幼逝寶寶收錄原則（2026-07-14 起）**：出生後未滿一歲即夭折的個體，**只要有正式命名就照常收錄、上站**（如 `takeru`、`tsubasa`、`wu-tan`）；唯有從未取名、僅以佔位名（如 `Baby`／`赤ちゃん`）登錄者，才移入 `wiki/_hidden/` 暫藏。此規則**取代** 2026-07-02「未滿一歲一律暫藏」的舊做法。（注意：因其他原因暫藏者不受此規則影響，如資料未經核實的 `sokka`。另，**佔位期間夭折的蘋果籽**改為「當季保留、季末（12/1 排程 `hide-deceased-apple-seeds-yearly`）才移入 `_hidden`」，見上方「蘋果籽」章節之「佔位期間夭折」，2026-07-21 起。）
