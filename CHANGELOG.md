...
`OFFICIAL_HOSTS` 補 **`zoo-leipzig.de`／`www.zoo-leipzig.de`**（園方公告網址為
`/artikel/標題-slug-NNN/`，另有幼獸一覽 `/tiere-erlebniswelten/jungtiere/`）。未補之前
`audit.py` 會把這 14 筆園方公告列為「🟡 sources 有非官方 host」，且個體頁不會把它們認成官方來源。

---

## 2026-09-01 ・年度回顧三處改版：小紀錄修正、曲線選年器、提問式入口

1. **修正「最多寶寶誕生的園」抓錯**（`Review.astro`）：08-31 出生分組改成時間順序後，小紀錄仍取第一組，
   語意變成「最早有寶寶的園」。51 個年份有 **35 年顯示錯**（如 2015 顯示 SCBI 2 隻，實為台北 4 隻）。
   改為真正取隻數最多；同數並列時全部列出。
2. **熱絡園平手規則**（`data.js`）：同總數時改「出生數優先」再比園 id——出生比搬家往來難得
   （2015 台北 4 生 0 搬曾被純 id 平手擠出 top5，現為第五名）。
3. **索引頁曲線選年器**：「逐年回顧」的 51 張年份小卡改成一條「該年紀錄總數」的平滑曲線
   （Catmull-Rom→bezier、零 client JS）：每年圓點＋整欄熱區直達年頁，title／aria-label 帶 🐣🪐🚐 摘要；
   沒紀錄的年份只畫小灰點不可點；sr-only 年份清單保留當無障礙後備。
4. **提問式入口**：首頁紀念日卡與 `/review/` 索引頁各加一排提問 chips（最多寶寶？最多搬家？
   男寶／女寶最多？最多去小熊星球？）——**不揭答案**，點了直達那一年。只比已完結年份；
   性別題加「不詳 ≤ 一成」的自保條件。年頁出生節同步新增性別小計「其中 ♀ n・♂ n」（同一條件）。
5. i18n +8 key（542 → 550 × 5 語）。`SHOW_REVIEW` 仍為 dev-only，公開時機由維護者決定。

驗收：`check_i18n` ✅、`SKIP_OG` build 7,032 頁 ✅、`closed.test` 68 passed ✅、dev server 截圖（索引／2015／首頁）✅。

---

## 2026-08-31 ・`/irpd/` 代表例增補：池田動物園（第 4 園）

日本段〈代表例〉加入 **池田動物園**（`zoos.json` id 15、岡山市，站內住民シズク／キナコ）。
選它的理由是**規模對照**：前三園分別代表「由來寫得最清楚」「檔期最長、五類全包」「排在當天且有互動企劃」，
池田補上光譜的另一端——整個活動就是當天一場飼育員特別ガイド，雨天即中止，五類裡只做第一類。

動到的檔案：`irpd-intro.md`（中文正本＋原文備存註解）、`web/src/components/Irpd.astro`（`EXAMPLES` 加 `{ zoo: 15 }`）、
`pipeline/src/i18n/*.json`（`irpd_zoo_15` × 5 語，539 → 542 key）、`docs/irpd-page-PLAN.md`（代表例表格加一列、候補清單移除池田）。

⚠️ **時間與場所只寫在海報圖上**。活動頁本文與側欄卡片都只有一句「飼育員が普段のガイドより詳しく…」與日期 `9/19`；
`14:00〜`／`レッサーパンダ舎`／`※雨天中止` 三項是實開海報 JPG 逐字讀來的（`wp-content/uploads/2026/08/2026国際レッサーパンダの日-1.jpg`）。
海報原文已抄進 `irpd-intro.md` 的 HTML 註解備存。

⚠️ **連結沿用大森山的作法連年度活動頁**（維護者 2026-08-31 裁定：與其他園一致）。
PLAN §2.3 提醒年度頁檔期結束後常下架，故備存註解裡寫明退路：改連站內園頁或 `https://ikedazoo.jp/event/`。
...
**① 親屬自動展開沒有終止條件。** 舊規則（`CLAUDE.md`〈新增成員流程〉第 3 步、
`rpf-wiki-SKILL.md` 第三步）寫「父、母、雙胞胎、子女、兄弟姊妹、祖父母，若無條目一律建立」，
但每一隻新建出來的親屬又各自帶著同一份清單：主角 → 子女 → 子女的另一位親本 → 那位親本的父母
→ ⋯⋯ 一路展開不會停。實際做 Pavitra 時是靠人為判斷停在 Pabu，不是靠規則，
同一份回報在不同時間做範圍會不一樣。

- 改為 **展開深度上限＝一圈**：建 父／母／同胎手足／兄弟姊妹／子女／**配偶**；
  **祖父母不再自動建**，新建條目自己的父母子女手足一律純文字＋RPF ID、**不遞迴**。
  （新增「配偶」是因為子女的另一位親本若缺條目，`web/src/lib/data.js` 判血緣度時
  兩邊父親都是 `undefined`，全血緣手足會被顯示成 ½——見 `SCHEMA.md`〈幽靈親代〉。）
- 每一隻另加**建檔門檻**：`born` 有可寫成 slug 的粒度（完整日期或年份）**且**有正式名字。
- **範圍外撈到的個體一律列進回報問維護者，不擅自建。**

**② `born` 與其他欄位其實不同級，但規則沒講。** `born` 是 slug 的一部分，寫錯要走
`tools/rename.py` 做全站遷移（檔名＋全 wiki `[[wikilink]]`＋`siblings:`＋index＋log，
還要人工補標題／引言／內文的舊名字）；其他欄位寫錯都只是原地改一行。

- 新增規則：**開檔前先把 `born` 釘死**。來源優先序＝出生園當時的一手來源（新聞稿／園報／
  官方社群）＞ 收容園日後撰寫的介紹頁 ＞ RPF。回報者或現居園給的生日先核一次出生園的公告。
- 一併釐清一個被誤用的判準：〈官方來源可直接採用〉裡「官方之間衝突留待維護者裁定」
  是給**同級**來源用的。**一手 vs 事後轉述不同級**，可自行判斷並把差異寫進備注。
  實例即本輪的 Pavitra——ZooMontana 官網動物頁記 `June 1, 2023`、出生園 San Diego Zoo
  Wildlife Alliance 命名新聞稿記 `born June 9`，採 6/9。

改動檔案：`CLAUDE.md`〈新增成員流程〉（步驟由 8 條變 9 條）、`rpf-wiki-SKILL.md`
（第三步全面改寫、第四步前置新段落、注意事項兩則）。
順手修：`rpf-wiki-SKILL.md` 的已故標記仍寫 🪐，補正為 🪽（2026-08-08 全庫已改，skill 漏跟）。

**② 的成本上限（同日兩改）**：②第一版只寫「核對一次」，沒說**哪些個體不該去核**——
跟①原本的毛病同一個形狀，所以補了適用範圍。但**第一版的適用範圍寫錯了**，用年代線切
（「1990 年代前的日本個體 → 不另搜」）。維護者當場指出「有些確實是翻 Wayback 才看到的」，
實測後證實：**全庫 200 筆條目以 `web.archive.org` 為來源，其中 169 筆生年在 2000 年之前**——
那條規則正好把最需要翻 Wayback 的族群排除掉。

改為**依管道、不依年代**：

- 找的順序＝現行官網／官方社群 → 該園既有管道（園報／staff blog／年度存檔）→ **Wayback**。
  Wayback 是頁面 404／CMS 改版下架／園已閉園時的**正規手段、不是最後手段**（既有先例：
  茶臼山 `/zukan/zukan/YYYY`、池田訃報、のいち舊個體頁、平川舊 blog、静岡市舊 PDF），
  快照仍算官方一手、佐證權重照舊。
- **可跳過的只有兩種**：① 出生園不詳（沒有搜索目標）；② 該園該年代確實沒有對外紀錄管道
  **且專案先前已確認過**（釧路《あゆみ》未數位化、安佐飼育記録集掃描無文字層）——
  第一次遇到某座園不算，先搜一輪再把結論記進 memory。
- **成本上限**：三層各搜一輪，沒有就停、照最強來源落檔並在備注寫「僅 X 為據」，不做第二輪。
  界限放在「搜幾輪」而不是「哪些個體不准搜」。

**未動的部分**：這兩條都只寫進文件，`audit.py`／`verify.sh` 沒有對應的機器檢查——
展開範圍與生日來源都不是機器判得出來的東西。
